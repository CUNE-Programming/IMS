{{--
file: resources/views/components/layouts/coordinator.blade.php
author: Ian Kollipara
date: 2024-08-05
description: The primary wrapper for all views in the coordinator panel.
 --}}
        ...
        <ul class="flex flex-1 flex-col gap-7"
          ...
          <li>
            <ul class="-m-2 space-y-1"
                role="list">
              <x-nav.item route="coordinator.seasons.index"
                          icon="trophy">
                Manage Seasons
              </x-nav.item>
              <x-nav.item route="coordinator.seasons.create"
                          icon="star">
                Start New Season
              </x-nav.item>
            </ul>
          ...
          <li class="mt-auto">
            <form class="flex w-full"
                  action="{{ route("coordinator.login.destroy") }}"
                  method="post">
              @csrf
              @method("DELETE")
